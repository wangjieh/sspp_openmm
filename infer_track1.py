...
def run_inference(model: Net, split_root: Path, norm_cfg: Dict[str, float], threshold: float, min_area: int,
                  ...
                  require_masks: bool = True, sequence_names: list[str] | None = None,
                  show_progress: bool = True, progress_desc: str = "Inference") -> Dict[str, float]:
    """Run inference for a complete split or an explicitly assigned sequence subset.

    ``sequence_names`` lets DDP workers process disjoint sequences while sharing
    the same output directory. A sequence is written by exactly one worker.
    """
    sequence_index = discover_sequences(split_root, require_masks=require_masks)
    if sequence_names is not None:
        known_sequences = set(sequence_index)
        unknown = sorted(set(sequence_names) - known_sequences)
        if unknown:
            raise ValueError(f"Unknown sequence(s) requested for inference: {unknown[:3]}")
        active_sequence_names = list(sequence_names)
    else:
        active_sequence_names = list(sequence_index)
    output_dir.mkdir(parents=True, exist_ok=True)
    total_tp = total_fp = total_fn = 0
    model.eval()

    with torch.no_grad():
        sequence_progress = tqdm(active_sequence_names, total=len(active_sequence_names), desc=progress_desc,
                                 dynamic_ncols=True, leave=True, disable=not show_progress)
        for sequence_name in sequence_progress:
            dataset = Track1SequenceDataset(split_root, sequence_name, norm_cfg, require_masks=require_masks,
            ...
            for batch in loader:
                ...
                lines.append(submission_line(frame_id, predicted_centroids))
    ...
    results = {"tp": total_tp, "fp": total_fp, "fn": total_fn}
    if require_masks:
        results["f1_proxy"] = f1_from_counts(total_tp, total_fp, total_fn)
        precision_denominator = total_tp + total_fp
        recall_denominator = total_tp + total_fn
        results["precision_proxy"] = 0.0 if precision_denominator == 0 else total_tp / precision_denominator
        results["recall_proxy"] = 0.0 if recall_denominator == 0 else total_tp / recall_denominator
    return results
...
def main():
    ...
    require_masks = (split_root / next(iter(discover_sequences(split_root, require_masks=False))) / "mask").is_dir()
    model = Net(model_name=model_name).to(device)
    model.load_state_dict(checkpoint["state_dict"])
    results = run_inference(model, split_root, checkpoint["norm_cfg"], threshold, args.min_area, args.match_distance,
                            args.output_dir, device, args.num_workers, require_masks=require_masks)
    ...
    if args.zip_path:
        ...
        if Path(archive) != args.zip_path:
            Path(archive).replace(args.zip_path)
