...
from centroid_metrics import f1_from_counts
from dataset_track1 import Track1TrainDataset, compute_normalization, discover_sequences
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train RFR using the CSIG Track 1 train/val layout.")
    ...
    parser.add_argument("--batch-size", type=int, default=2, help="Per-GPU batch size under torchrun.")
    ...
    parser.add_argument("--log-interval", type=int, default=20, help="Update tqdm loss/GPU-memory statistics every N batches.")
...
def memory_summary(device: torch.device) -> str:
    if device.type != "cuda":
        return "cpu"
    allocated = gibibytes(torch.cuda.memory_allocated(device))
    reserved = gibibytes(torch.cuda.memory_reserved(device))
    peak = gibibytes(torch.cuda.max_memory_allocated(device))
    return f"{allocated:.2f}/{reserved:.2f}G peak {peak:.2f}G"


def save_checkpoint(path: Path, model, optimizer, scheduler, epoch: int, best_f1: float, norm_cfg, args):
    path.parent.mkdir(parents=True, exist_ok=True)
    torch.save({"epoch": epoch, "state_dict": unwrap_model(model).state_dict(), "optimizer": optimizer.state_dict(),
                "scheduler": scheduler.state_dict(), "best_f1": best_f1, "norm_cfg": norm_cfg,
                "model_name": args.model_name, "threshold": args.threshold, "min_area": args.min_area,
                "match_distance": args.match_distance}, path)


def global_mean_loss(losses, device: torch.device, distributed: bool) -> float:
    summary = torch.tensor([float(sum(losses)), len(losses)], dtype=torch.float64, device=device)
    if distributed:
        dist.all_reduce(summary, op=dist.ReduceOp.SUM)
    return float(summary[0].item() / max(summary[1].item(), 1.0))


def global_validation_metrics(local_metrics, device: torch.device, distributed: bool):
    """Sum per-rank validation counts and calculate the metric for all sequences."""
    counts = torch.tensor(
    ...
    tp, fp, fn = (int(value) for value in counts.tolist())
    precision_denominator, recall_denominator = tp + fp, tp + fn
    return {
        "tp": tp,
        "fp": fp,
        "fn": fn,
        "f1_proxy": f1_from_counts(tp, fp, fn),
        "precision_proxy": 0.0 if precision_denominator == 0 else tp / precision_denominator,
        "recall_proxy": 0.0 if recall_denominator == 0 else tp / recall_denominator,
    }
...
def main():
    ...
    try:
        ...
        if args.resume:
            ...
            start_epoch, best_f1 = int(checkpoint["epoch"]), float(checkpoint.get("best_f1", -1.0))
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            progress = tqdm(train_loader, total=len(train_loader), desc=f"Epoch {epoch + 1}/{args.epochs}",
                            dynamic_ncols=True, leave=True, disable=not is_main_process(rank))
            for step, (images, masks) in enumerate(progress, start=1):
                images, masks = images.to(device, non_blocking=True), masks.to(device, non_blocking=True)
                optimizer.zero_grad(set_to_none=True)
                loss = model(images, masks)  # DDP forward: gradients synchronize across GPUs.
                loss.backward()
                optimizer.step()
                loss_value = float(loss.detach().cpu())
                losses.append(loss_value)
                interval_losses.append(loss_value)
                if is_main_process(rank) and (step % args.log_interval == 0 or step == len(train_loader)):
                    progress.set_postfix(loss=f"{loss_value:.5f}", avg=f"{np.mean(interval_losses):.5f}",
                                         mem=memory_summary(device), lr=f"{optimizer.param_groups[0]['lr']:.2e}")
            ...
            if (epoch + 1) % args.val_every == 0:
                # Every rank validates a disjoint subset.  Keeping seven ranks at
                # a barrier while rank 0 validated the full split caused the NCCL
                # watchdog timeout seen with long validation sets.
                validation_sequences = list(discover_sequences(args.data_root / "val", require_masks=True))
                local_sequences = validation_sequences[rank::world_size]
                local_metrics = run_inference(
                    unwrap_model(model), args.data_root / "val", train_dataset.norm_cfg, args.threshold,
                    args.min_area, args.match_distance, args.output_dir / "validation_txt", device,
                    args.num_workers, require_masks=True, sequence_names=local_sequences,
                    show_progress=is_main_process(rank),
                    progress_desc=f"Validation {epoch + 1}/{args.epochs} (rank 0: {len(local_sequences)} seqs)",
                )
                metrics = global_validation_metrics(local_metrics, device, distributed)
                record.update(metrics)
                if is_main_process(rank) and metrics["f1_proxy"] > best_f1:
            ...
            barrier(distributed)
            if is_main_process(rank):
                save_checkpoint(args.output_dir / "last.pth.tar", model, optimizer, scheduler, epoch + 1,
                                best_f1, train_dataset.norm_cfg, args)
